import { Settings, ArrowUpCircle, Cpu, Eye, Clock } from 'lucide-react';
...
const NAV_ITEMS: { id: AppTab; icon: typeof Cpu; label: string }[] = [
  { id: 'agents', icon: Cpu, label: 'Agents' },
  { id: 'observability', icon: Eye, label: 'Observe' },
...
export default function SidebarFooter({
  ...
  return (
    <div className="flex-shrink-0" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      {/* Mobile nav */}
      <div className="space-y-1.5 px-3 pt-2 md:hidden">
        {NAV_ITEMS.map(({ id, icon: Icon, label }) => (
          <button
            key={id}
            className={`flex h-11 w-full items-center gap-3.5 rounded-xl px-4 transition-all active:scale-[0.98] ${
              activeTab === id
                ? 'bg-accent text-foreground'
